...
slogon() {
    ...
    echo -e "# ${GREEN}作者${PLAIN}: 网络跳越(hijk) & Harvey                               #"
    echo -e "# ${GREEN}网址${PLAIN}: https://Harvey.art                                    #"
    echo -e "# ${GREEN}论坛${PLAIN}: https://Harvey.club                                   #"
    echo -e "# ${GREEN}TG群${PLAIN}: https://t.me/Harveyclub                               #"
    echo "#############################################################"
    echo ""
}
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        curl -sL https://get.acme.sh | sh -s email=harvey.pw@protonmail.ch
        source ~/.bashrc
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，请复制上面的红色文字联系Harvey"
            exit 1
        }
        CERT_FILE="/etc/v2ray/${DOMAIN}.pem"
        KEY_FILE="/etc/v2ray/${DOMAIN}.key"
        ...
        [[ -f $CERT_FILE && -f $KEY_FILE ]] || {
            colorEcho $RED " 获取证书失败，请联系Harvey"
            exit 1
        }
    else
...
installV2ray() {
    ...
    if [[ ! -f $CONFIG_FILE ]]; then
        colorEcho $RED " $OS 安装V2ray失败，请联系Harvey"
        exit 1
    fi
...
installNginx() {
    if [[ "$BT" = "false" ]]; then
        ...
        res=$(command -v nginx)
        if [[ "$res" = "" ]]; then
            colorEcho $RED " Nginx安装失败，请联系Harvey"
            exit 1
        fi
