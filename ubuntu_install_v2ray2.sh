...
installV2ray() {
    ...
    if [ ! -f $CONFIG_FILE ]; then
        colorEcho $RED " $OS 安装V2ray失败，请联系Harvey反馈"
        exit 1
    fi
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，请联系Harvey反馈"
            exit 1
        }
        CERT_FILE="/etc/v2ray/${DOMAIN}.pem"
        KEY_FILE="/etc/v2ray/${DOMAIN}.key"
...
installNginx() {
    ...
    res=$(command -v nginx)
    if [[ "$res" = "" ]]; then
        colorEcho $RED " Nginx安装失败，请联系Harvey反馈"
        exit 1
    fi
...
EOF
    ...
    if [ "${res}" = "" ]; then
        echo -e " $OS nginx启动失败！ 请到 ${RED}https://Harvey.art${PLAIN} 反馈"
        exit 1
    fi
}
